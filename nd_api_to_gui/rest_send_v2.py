...
"""
Send REST requests to the controller with optional retries.
"""
...
class RestSend:
    ...
    def __init__(self, params) -> None:
        ...
        self._result: list = []
        self._result_current: dict[str, bool] = {}
        ...
        self.saved_timeout: int = self._timeout
        self.saved_check_mode: bool = self._check_mode

        self._valid_verbs: set[str] = {"GET", "POST", "PUT", "DELETE"}
    ...
    def restore_settings(self) -> None:
        ...
        -   `check_mode` is not restored if `save_setting()` has not previously been called.
        -   `timeout` is not restored if `save_setting()` has not previously been called.
        """
    ...
    def save_settings(self) -> None:
        ...
        Save the current values of `check_mode` and `timeout` for later restoration.
    ...
    def commit_check_mode(self) -> None:
        ...
        if not self.response_handler:
            msg = f"{self.class_name}.{method_name}: "
            msg += "response_handler must be set prior to calling "
            msg += f"{self.class_name}.{method_name}"
            self.log.error(msg)
            raise ValueError(msg)

        response_current: dict[str, Any] = {}
        ...
        try:
            ...
            self._response.append(copy.deepcopy(self.response_current))
            self._result.append(copy.deepcopy(self.result_current))
        except (TypeError, ValueError) as error:
            msg = f"{self.class_name}.{method_name}: "
    ...
    def commit_normal_mode(self) -> None:
        ...
        Call Sender() with retries until successful response or timeout is exceeded.
        ...
        if self.sender is None:
            msg = f"{self.class_name}.{method_name}: "
            msg += "sender must be set prior to calling "
            msg += f"{self.class_name}.{method_name}"
            self.log.error(msg)
            raise ValueError(msg)
    ...
    def response_handler(self) -> Any:
        ...
        A class instance that implements the response handler interface.  This
        ...
        -   See DCNM Ansible Collection plugins/module_utils/common/response_handler.py
            for details about implementing a `ResponseHandler` class.
        """
        return self._response_handler

    @response_handler.setter
    def response_handler(self, value) -> None:
        method_name: str = inspect.stack()[0][3]
        _implements_need: str = "response_handler_v1"
        _implements_have: str = "NOT_SET"
        msg = f"{self.class_name}.{method_name}: "
    ...
    @property
    def result(self) -> list[dict[str, bool]]:
        """
        # Summary
    ...
    def result_current(self, value: dict[str, bool]) -> None:
        method_name: str = inspect.stack()[0][3]
        if not isinstance(value, dict):
            msg = f"{self.class_name}.{method_name}: "
            msg += f"{method_name} must be a dict. "
